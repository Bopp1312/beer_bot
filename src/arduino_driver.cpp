...
private:
  ...
  void update_odom()
  {
    ...
    tf_msg.transform.translation.y = current_position.y;
  ...
  void read_serial()
  {
    ...
    if (bytes_read > 0)
    {
      ...
      encoder_right_buffer[bytes_read - comma_index - 1] = '\0';

      try {
        current_encoder.left = std::stoi(encoder_left_buffer);
        current_encoder.right = std::stoi(encoder_right_buffer);
      } catch (...) {
        RCLCPP_ERROR(this->get_logger(), "Invalid encoder reading");
        return;
      }
